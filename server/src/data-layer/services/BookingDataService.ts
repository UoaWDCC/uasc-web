...
export default class BookingDataService {
  ...
  public async getBookingsByUserId(userId: string): Promise<Booking []> {
    const result = await FirestoreCollections.bookings.where("user_id", "==", userId).get()
  ...
  public async updateBooking(bookingslotId: string, bookingData: Booking) {
    return await FirestoreCollections.bookings.doc(bookingslotId).update(bookingData)
  }
